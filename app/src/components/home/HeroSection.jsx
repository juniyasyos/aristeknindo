      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 pt-16 pb-8 sm:pt-24 sm:pb-16 lg:py-24 w-full">
        <div className="max-w-2xl lg:max-w-3xl">
          ...
          >
            ...
            <span className="font-mono text-[10px] sm:text-xs text-brand-redLight font-medium tracking-wide uppercase truncate">
              {lang === 'id' ? data.badgeId : data.badgeEn}
            </span>
          </motion.div>

          {/* H1 Headline: Refined font size on mobile */}
          <motion.h1
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6, delay: 0.2 }}
            className="text-xl sm:text-4xl lg:text-5xl font-bold sm:font-extrabold text-white tracking-tight leading-snug sm:leading-[1.18] mb-2 sm:mb-5"
          >
          ...
          <motion.p
            ...
            className="text-[11px] sm:text-base lg:text-lg text-slate-300 leading-relaxed mb-4 sm:mb-8 max-w-xl line-clamp-2 sm:line-clamp-none"
          >
          ...
          <motion.div
            ...
            className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3 w-full sm:w-auto mb-4 sm:mb-10"
          >
            <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="w-full sm:w-auto">
              <Link
                to="/contact"
                className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-4 py-2.5 sm:px-6 sm:py-3.5 rounded-lg bg-brand-red hover:bg-brand-redDark text-white font-semibold text-xs sm:text-sm shadow-lg shadow-brand-red/30 transition-all group text-center"
              >
            ...
            <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="w-full sm:w-auto">
              <a
                ...
                className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-4 py-2.5 sm:px-6 sm:py-3.5 rounded-lg bg-canvas/90 hover:bg-canvas backdrop-blur-md border border-slate-border text-white text-xs sm:text-sm font-medium hover:border-slate-muted transition-all text-center"
              >
                <MessageSquare className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-emerald-400 flex-shrink-0" />
                <span>{lang === 'id' ? data.ctaWhatsappId : data.ctaWhatsappEn}</span>
              </a>
            </motion.div>
          </motion.div>

          {/* Key Quick Highlights Bar: Refined text size on mobile */}
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6, delay: 0.65 }}
            className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-4 pt-3 sm:pt-6 border-t border-slate-border/60 max-w-2xl bg-canvas-card/40 sm:bg-transparent backdrop-blur-sm sm:backdrop-blur-none rounded-xl p-2 sm:p-0 border sm:border-0 border-slate-border/50"
          >
            {data.stats.map((stat, idx) => (
              ...
              >
                <span className="font-mono text-xs sm:text-xl font-bold text-white tracking-tight">
                  {stat.value}
                </span>
                <span className="text-[9px] sm:text-xs text-slate-300 mt-0.5 leading-tight truncate sm:whitespace-normal">
